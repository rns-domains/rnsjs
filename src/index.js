...
import { formatsByName } from '@rnsdomains/address-encoder'
import { abi as rnsContract } from '@rnsdomains/rns/build/contracts/rns.json'
import { abi as resolverContract } from '@rnsdomains/resolver/build/contracts/Resolver.json'
import { abi as reverseRegistrarContract } from '@rnsdomains/rns/build/contracts/ReverseRegistrar.json'
...
function getrnsAddress(networkId) {
  if ([2611].includes(parseInt(networkId))) {
    return '0xA00edba30dD5F07fE4756988544E1b7613655002'
  }
}
...
function getRNSContract({ address, provider }) {
  return new ethers.Contract(address, rnsContract, provider)
}
...
class Name {
  constructor(options) {
    const { name, rns, provider, signer, namehash: nh, resolver } = options
    if (options.namehash) {
      this.namehash = nh
    }
    this.rns = rns
    this.rnsWithSigner = this.rns.connect(signer)
    this.name = name
  ...
  async getOwner() {
    return this.rns.owner(this.namehash)
  }

  async setOwner(address) {
    if (!address) throw new Error('No newOwner address provided!')
    return this.rnsWithSigner.setOwner(this.namehash, address)
  }

  async getResolver() {
    return this.rns.resolver(this.namehash)
  }

  async setResolver(address) {
    if (!address) throw new Error('No resolver address provided!')
    return this.rnsWithSigner.setResolver(this.namehash, address)
  }

  async getTTL() {
    return this.rns.ttl(this.namehash)
  }
  ...
  async setSubnodeOwner(label, newOwner) {
    ...
    return this.rnsWithSigner.setSubnodeOwner(this.namehash, lh, newOwner)
  }

  async setSubnodeRecord(label, newOwner, resolver, ttl = 0) {
    const lh = labelhash(label)
    return this.rnsWithSigner.setSubnodeRecord(
      this.namehash,
      lh,
      newOwner,
      resolver,
...
export default class RNS {
  constructor(options) {
    const { networkId, provider, rnsAddress } = options
    let ethersProvider
    ...
    this.rns = getRNSContract({
      address: rnsAddress ? rnsAddress : registries[networkId],
  ...
  async getName(address) {
    ...
    const resolverAddr = await this.rns.resolver(namehash(reverseNode))
